...
- **`index.html`** — the **home page** in the site's *"warm craftsman modern"* design (light, warm, photo-first; see `docs/superpowers/specs/2026-09-23-warm-craftsman-redesign-design.md`): Murray blue as the single accent, a real-project photo hero, trust bar, services / three-step free-quote flow / work gallery / before-after / **meet the owner** / **reviews** (real Facebook recommendations + Google rating) / how-we-work / service-area map, and an in-page **Tweaks** panel (accent color, light/dark theme, display font).
- **`site/`** — the homepage's parts: `styles.css` (design tokens, light default + `html[data-theme="dark"]`, shared components), `pages.css` (sub-page components), `fonts.css` (self-hosted Bricolage Grotesque + Hanken Grotesk), `image-slot.js` (drop-in photo placeholder), `servicemap.js` (Leaflet service-area map), and React components (`Header`, `Hero`, `Sections`, `More` (about + reviews + map), `Contact` (three-step quote flow), `Footer`, `App`).
