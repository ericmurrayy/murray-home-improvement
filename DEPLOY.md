...
| `/portfolio/<slug>/` (all 11 old portfolio posts) and the photo attachment pages that belong to them (`/img-NNNN/`, listed as `legacy` in `scripts/projects.json`) | `/projects/<slug>.html` (generated by `scripts/gen-projects.mjs`) |
| `/project-type/bath/` | `/services/bathroom-remodeling.html` |
| `/project-type/kitchen/` | `/services/kitchen-remodeling.html` |
| `/project-type/additions/` | `/services/home-additions.html` |
| `/project-type/decks/` | `/services/decks-porches.html` |
| `/project-type/basement/` | `/services/basement-finishing.html` |
...
Ready to send (text or email, after the final walk-through):

> Hi <name>, thanks again for having me work on your <kitchen/bath/addition>. If you have a minute, a quick Google review helps the next Chelmsford neighbor find us: murrayhomeimprovement.com/review — Eric

> Subject: Thank you from Murray Home Improvement
> Hi <name>, it was a pleasure building your <project>. Reviews are how most of my work finds me, so if you were happy with how it went, would you leave a few words on Google? It takes about a minute: https://www.murrayhomeimprovement.com/review
> Thanks, Eric Murray · (978) 479-9406

## Project pages

`scripts/projects.json` is the source of truth for the 11 project pages (titles, summaries, captions, which photo is the cover, which old attachment page belongs to which project). `scripts/project-photos.mjs` makes the web copies of the photos (needs `sharp` and the photo archive in `~/Pictures/murray-old-website-photos`); `node scripts/gen-projects.mjs` then rewrites `projects/*.html`, the project blocks in `gallery.html` and the service pages, `sitemap.xml`, `llms.txt` and the portfolio/attachment redirects. Edit the JSON, run the script, commit the output. Town and year stay blank until Eric supplies them (no invented facts).
